...
# --- Exercise / recovery handling ---
# During genuine "exercise" motion class, HR can legitimately reach 180+ bpm
# (a 25-year-old's max HR is ~195). The default hr_very_high (150) would
# otherwise flag every vigorous workout as distress.
HR_VERY_HIGH_EXERCISE = 185.0  # bpm — exercise-aware cap (vs. 150 baseline)

# Post-exercise recovery: HR stays elevated for several minutes after
# vigorous activity. Suppress HR-only distress alerts during this window.
RECOVERY_COOLDOWN_S = 900.0    # seconds (15 min) of HR-only suppression after exercise
...
class DistressEngine:
    ...
    def __init__(self):
        ...
        # Recovery cool-down state — populated when motion drops out of
        # active/exercise so the next ~5 min of HR-only signals are
        # treated as physiological recovery, not new distress.
        self._last_motion: str = "still"
        self._recovery_until: float = 0.0   # epoch seconds; >0 means in cool-down
    ...
    def evaluate(self, pipeline_output: Dict) -> Dict:
        ...
        # ------------------------------------------------------------------
        # Step 3: Motion-aware suppression  (with exercise & recovery awareness)
        # ------------------------------------------------------------------
        # During exercise, elevated HR alone is expected and NOT stressful.
        # The cap depends on motion class:
        #   - "exercise" (vigorous, vm > 0.50) → use HR_VERY_HIGH_EXERCISE (185)
        #     because vigorous workouts routinely hit 160-180 bpm even in
        #     healthy young adults (max HR ≈ 220 - age).
        #   - "active" (moderate, vm 0.25-0.50) → use the standard cap (150).
        # SpO2 drops are never suppressed — those indicate genuine concern.
        hr = sample.get("hr")

        # Track motion transitions for recovery cool-down. When motion drops
        # out of "exercise" (vigorous workout, vm > 0.50), start the
        # cool-down window during which HR-only flags are suppressed.
        if (self._last_motion == "exercise" and motion_state != "exercise"):
            self._recovery_until = now + RECOVERY_COOLDOWN_S
        self._last_motion = motion_state

        # Only suppress for true "exercise" motion (vm > 0.50). The "active"
        # class (vm 0.25-0.50) covers things like agitated stress pacing and
        # household chores, which do NOT explain elevated HR by themselves.
        if motion_state == "exercise":
            if (flags.hr and not flags.spo2
                    and hr is not None and hr < HR_VERY_HIGH_EXERCISE):
                flags.hr = False
                flags.hr_score = 0.0
        elif now < self._recovery_until:
            # Recovery cool-down: HR is still decaying after exercise.
            # Suppress HR-only flags below the exercise cap so we don't
            # alert on a 140 bpm reading 90 seconds after a sprint.
            if (flags.hr and not flags.spo2
                    and hr is not None and hr < HR_VERY_HIGH_EXERCISE):
                flags.hr = False
                flags.hr_score = 0.0
        ...
        if hr_val is not None and hr_val >= ABSOLUTE_THRESHOLDS["hr_very_high"]:
            # Skip the emergency bypass during exercise OR while still in the
            # post-exercise recovery cool-down window. Bypass below the
            # exercise cap (185) because true emergencies (HR ≥ 185 at rest)
            # remain caught.
            in_exercise = motion_state == "exercise"
            in_recovery = now < self._recovery_until
            below_exercise_cap = hr_val < HR_VERY_HIGH_EXERCISE
            if not in_exercise and not (in_recovery and below_exercise_cap):
...
# ===========================================================================
# 14b. MANUAL TRIGGER & GRADUATED-TRUST ESCALATION
# ===========================================================================
"""
Manual SOS layer — sits ABOVE DistressEngine.evaluate() and implements the
graduated-trust cascade:

  Engine verdict at press           Branch                         Action
  -------------------------------------------------------------------------
  normal    → CONFIRM_USER          ask "Are you okay?"            60 s
                                    silent  → escalate T2 + T3     (inability)
                                    "safe"  → resolve
                                    "help"  → dispatch T4
  stress    → MID_ESCALATION        T1 immediate                   30 s
                                    PIN cancel → resolve, T1 informed
                                    timeout    → T2 then T3
  distress  → FULL_ESCALATION       T1 + T2 + T3 immediate         15 s
                                    PIN cancel → resolve (unless duress_flag)
                                    timeout    → T4 (emergency)

Design principles
-----------------
  • The button is always believed.  The engine decides loudness.
  • Silence is never interpreted as "user is fine."
  • PIN-cancel during severe distress is treated as coercion, not consent.
  • This module is stateful per-user; instantiate one ManualTriggerHandler
    per user_id and call its methods from the API / dashboard layer.

Lifecycle
---------
  IDLE
    └──► on_button_press(engine_state, …)
          ├──► PENDING_CONFIRMATION  (engine_state == "normal")
          │      ├──► on_user_action("safe")  → RESOLVED
          │      ├──► on_user_action("help")  → DISPATCHED (T4)
          │      └──► tick() timeout 60 s     → MID_ESCALATION (T2+T3)
          ├──► MID_ESCALATION        (engine_state == "stress")
          │      ├──► on_user_action("cancel")  → RESOLVED  (T1 informed)
          │      └──► tick() delays elapse      → T2, T3 fire
          └──► FULL_ESCALATION       (engine_state == "distress")
                 ├──► on_user_action("cancel")  → RESOLVED
                 │     (or DURESS_SILENT_CONTINUE if severity ≥ 0.85)
                 └──► tick() delay elapses      → T4 → DISPATCHED
"""
...
class ManualTriggerState:
    """Carrier for the SOS lifecycle. One instance per active SOS event."""

    # Phase enum (strings so they serialise cleanly to JSON / Redis)
    IDLE                 = "idle"
    PENDING_CONFIRMATION = "pending_confirmation"
    MID_ESCALATION       = "mid_escalation"
    FULL_ESCALATION      = "full_escalation"
    RESOLVED             = "resolved"
    DISPATCHED           = "dispatched"

    phase: str = IDLE
    engine_state_at_press: Optional[str] = None
    engine_severity_at_press: float = 0.0
    press_time: float = 0.0
    deadline: float = 0.0                                      # absolute epoch
    tiers_queued: List[Tuple[str, float]] = field(default_factory=list)
    tiers_fired: Set[str] = field(default_factory=set)
...
class ManualTriggerHandler:
    """Stateful SOS overlay — confidence-gated single-recipient model.

    Architecture:
      • There is exactly ONE recipient per case.
      • The recipient is decided by where the engine's confidence falls
        relative to two USER-CONFIGURED thresholds (set in onboarding):
            confidence < contact_threshold  → ASK_USER (low — could be misclick)
            contact_threshold ≤ confidence < police_threshold  → CONTACT
            confidence ≥ police_threshold   → EMERGENCY_SERVICES
      • The tiered T1/T2/T3/T4 ladder has been removed. Either a single
        emergency contact is dialled OR emergency services is — never both
        in sequence, and the user is never spammed by multiple parties.
    """

    # ── Timing constants (overridable) ────────────────────────
    CONFIRMATION_WINDOW_S    = 60.0   # ASK_USER  branch (low confidence)
    CONTACT_CANCEL_WINDOW_S  = 30.0   # CONTACT   branch (mid confidence)
    POLICE_CANCEL_WINDOW_S   = 15.0   # POLICE    branch (high confidence)

    # Defaults — user override these in their app settings
    DEFAULT_CONTACT_THRESHOLD = 0.50
    DEFAULT_POLICE_THRESHOLD  = 0.85

    # Severity threshold above which PIN-cancel is treated as duress
    DURESS_SEVERITY_FLOOR = 0.85

    def __init__(self) -> None:
        self.state = ManualTriggerState()
    ...
    def is_active(self) -> bool:
        return self.state.phase not in (
            ManualTriggerState.IDLE,
            ManualTriggerState.RESOLVED,
            ManualTriggerState.DISPATCHED,
        )

    def on_button_press(
        self,
        *,
        engine_state: Optional[str]   = None,    # kept for backward compat / logs
        engine_confidence: float      = 0.5,
        engine_severity: float        = 0.0,
        contact_threshold: float      = None,
        police_threshold: float       = None,
        emergency_contact: Optional[Dict] = None, # {"name": "...", "phone": "..."}
        modality_breakdown: Optional[Dict] = None,
        now: Optional[float] = None,
    ) -> Dict:
        """Confidence-gated decision — one branch, one recipient."""
        if now is None:
            now = time.time()
        if contact_threshold is None:
            contact_threshold = self.DEFAULT_CONTACT_THRESHOLD
        if police_threshold is None:
            police_threshold = self.DEFAULT_POLICE_THRESHOLD

        if self.is_active():
            return self._build_verdict(
                response_path="NOOP_ALREADY_ACTIVE",
                reason=f"sos_already_in_phase_{self.state.phase}",
            )

        s = self.state
        s.press_time               = now
        s.engine_state_at_press    = engine_state
        s.engine_severity_at_press = engine_severity

        # Recipient decision — the heart of the new model
        if engine_confidence < contact_threshold:
            recipient = {"target": "ask_user", "label": "user confirmation"}
            phase     = ManualTriggerState.PENDING_CONFIRMATION
            window    = self.CONFIRMATION_WINDOW_S
            ui_prompt = "Are you okay? Tap green to dismiss, red to call for help."
            severity_out = 0.40
        elif engine_confidence < police_threshold:
            recipient = {
                "target": "contact",
                "label":  (emergency_contact or {}).get("name", "Emergency contact"),
                "phone":  (emergency_contact or {}).get("phone", ""),
            }
            phase     = ManualTriggerState.MID_ESCALATION   # phase string preserved for compatibility
            window    = self.CONTACT_CANCEL_WINDOW_S
            ui_prompt = f"Notifying {recipient['label']} in {int(window)} s. Tap to cancel (PIN required)."
            severity_out = 0.75
        else:
            recipient = {"target": "emergency_services", "label": "Emergency services (112 / 911)"}
            phase     = ManualTriggerState.FULL_ESCALATION  # phase string preserved
            window    = self.POLICE_CANCEL_WINDOW_S
            ui_prompt = f"EMERGENCY — calling 112 in {int(window)} s."
            severity_out = 1.00

        s.phase       = phase
        s.deadline    = now + window
        s.duress_flag = (recipient["target"] == "emergency_services" and
                         engine_severity >= self.DURESS_SEVERITY_FLOOR)
        s.reason      = f"manual_sos_conf={engine_confidence:.2f}_recipient={recipient['target']}"
        s.tiers_queued = []          # no queued cascade in the new model

        return self._build_verdict(
            response_path         = recipient["target"].upper(),
            cancellation_window_s = window,
            recipient             = recipient,
            engine_confidence     = engine_confidence,
            contact_threshold     = contact_threshold,
            police_threshold      = police_threshold,
            tiers_immediate       = [],   # legacy field — empty in new model
            tiers_queued          = [],
    ...
    # ── Convenience: decide recipient WITHOUT firing an SOS ──
    @classmethod
    def decide_recipient(
        cls,
        ...
        """Pure decision function — useful for the dashboard to preview
        who WOULD be notified before the button is actually pressed."""
        ct = contact_threshold if contact_threshold is not None else cls.DEFAULT_CONTACT_THRESHOLD
        pt = police_threshold  if police_threshold  is not None else cls.DEFAULT_POLICE_THRESHOLD
        if engine_confidence < ct:
            return {"target": "ask_user", "label": "user confirmation"}
        if engine_confidence < pt:
            return {
                "target": "contact",
                "label":  (emergency_contact or {}).get("name", "Emergency contact"),
                "phone":  (emergency_contact or {}).get("phone", ""),
            }
        return {"target": "emergency_services", "label": "Emergency services (112 / 911)"}

    def on_user_action(self, action: str, now: Optional[float] = None) -> Dict:
        """User responded to the modal / pressed cancel.

        Args:
            action: "safe" | "help" | "cancel" — what the user did.
                    "safe"   → only meaningful in PENDING_CONFIRMATION
                    "help"   → only meaningful in PENDING_CONFIRMATION
                    "cancel" → only meaningful in MID_/FULL_ESCALATION
                               (caller is responsible for PIN verification
                                BEFORE calling this method).

        Returns:
            verdict dict; phase will be RESOLVED, DISPATCHED, or unchanged.
        """
        if now is None:
            now = time.time()
        s = self.state
        s.last_user_action = action

        # ── PENDING_CONFIRMATION exits
        if s.phase == ManualTriggerState.PENDING_CONFIRMATION:
            if action == "safe":
                s.phase = ManualTriggerState.RESOLVED
                return self._build_verdict(
            ...
            if action == "help":
                s.phase = ManualTriggerState.DISPATCHED
                s.tiers_fired.add("T4")
                return self._build_verdict(
                    response_path   = "DISPATCH_T4",
                    reason          = "user_confirmed_help_needed",
                    tiers_immediate = ["T4"],
                    severity        = 1.0,
                )

        # ── MID_ / FULL_ESCALATION cancel
        if s.phase in (ManualTriggerState.MID_ESCALATION,
                       ManualTriggerState.FULL_ESCALATION):
            if action == "cancel":
                # Duress detection — cancel during severe distress = coercion
                if s.duress_flag and s.phase == ManualTriggerState.FULL_ESCALATION:
                    # UI shows "CANCELLED" to the attacker; engine silently continues
                    return self._build_verdict(
                        response_path = "DURESS_SILENT_CONTINUE",
                        reason        = "cancel_during_severe_distress_assumed_duress",
                        ui_display    = "CANCELLED",
                        severity      = 1.0,
                    )
                s.phase = ManualTriggerState.RESOLVED
                return self._build_verdict(
                    response_path = "RESOLVED",
                    reason        = f"user_pin_cancelled_phase_{s.phase}",
                    severity      = 0.0,
                )

        return self._build_verdict(
            response_path = "NOOP",
            reason        = f"action_{action}_ignored_in_phase_{s.phase}",
        )

    def tick(self, now: Optional[float] = None) -> Optional[Dict]:
        """Drive timers forward.  Call once per second from the engine loop.

        Returns:
            A verdict dict if anything changed (timeout fired / queued tier
        ...
        # ── ASK_USER timeout — silence escalates to CONTACT (not to police).
        if s.phase == ManualTriggerState.PENDING_CONFIRMATION:
            if now >= s.deadline:
                # Promote to CONTACT branch immediately
                s.phase    = ManualTriggerState.MID_ESCALATION
                s.deadline = now + self.CONTACT_CANCEL_WINDOW_S
                return self._build_verdict(
                    response_path  = "CONFIRMATION_TIMEOUT_NOTIFY_CONTACT",
                    reason         = "user_silent_60s_silence_is_inability",
                    recipient      = {"target": "contact", "label": "Emergency contact"},
                    severity       = 0.85,
                )

        # ── CONTACT branch deadline — dial the contact
        if s.phase == ManualTriggerState.MID_ESCALATION:
            if now >= s.deadline:
                s.phase = ManualTriggerState.DISPATCHED
                return self._build_verdict(
                    response_path = "CONTACT_DIALED",
                    reason        = "contact_cancel_window_elapsed",
                    severity      = 0.85,
                )

        # ── POLICE branch deadline — call emergency services
        if s.phase == ManualTriggerState.FULL_ESCALATION:
            if now >= s.deadline:
                s.phase = ManualTriggerState.DISPATCHED
                return self._build_verdict(
                    response_path = "EMERGENCY_SERVICES_DIALED",
                    reason        = "police_cancel_window_elapsed",
                    severity      = 1.0,
                )
    ...
    def status(self, now: Optional[float] = None) -> Dict:
        """Snapshot of the current SOS lifecycle (for /sos/status endpoint)."""
        if now is None:
            now = time.time()
        s = self.state
        return {
            "phase":                  s.phase,
            "engine_state_at_press":  s.engine_state_at_press,
            "press_time":             s.press_time,
            "deadline":               s.deadline,
            "seconds_to_deadline":    max(0.0, s.deadline - now) if s.deadline else 0.0,
            "tiers_fired":            sorted(s.tiers_fired),
            "tiers_queued":           s.tiers_queued,
            "duress_flag":            s.duress_flag,
            "last_user_action":       s.last_user_action,
            "is_active":              self.is_active(),
        }

    def _build_verdict(self, **kw) -> Dict:
        s = self.state
        out = {
            "phase":                  s.phase,
            "response_path":          kw.get("response_path", "NOOP"),
            "engine_state_at_press":  s.engine_state_at_press,
            "tiers_fired":            sorted(s.tiers_fired),
            "tiers_queued":           s.tiers_queued,
            "duress_flag":            s.duress_flag,
            ...
            "severity":               kw.get("severity", 0.0),
            "weight":                 1.0,   # manual trigger override weight
        }
        for k in ("cancellation_window_s", "tiers_immediate",
                  "modality_breakdown", "ui_display", "ui_prompt",
                  "recipient", "engine_confidence",
                  "contact_threshold", "police_threshold"):
            if k in kw:
                out[k] = kw[k]
        return out


# ── Hook into DistressEngine ─────────────────────────────────────────────
#
# We attach a ManualTriggerHandler to every DistressEngine instance so that
# the API / dashboard layer can do:
#
#     verdict = engine.evaluate(pipeline_output)
#     if user_pressed_button:
#         sos = engine.manual_trigger.on_button_press(
#             engine_state    = verdict["state"],
#             engine_severity = verdict.get("confidence", 0.0),
#         )
#     elif user_response:
#         sos = engine.manual_trigger.on_user_action(user_response)
#     else:
#         sos = engine.manual_trigger.tick()
#
# Done via monkey-patch of __init__ rather than editing the class body, so
# this whole block stays appendable without touching the engine above.

_original_distress_engine_init = DistressEngine.__init__

def _distress_engine_init_with_sos(self) -> None:
    _original_distress_engine_init(self)
    self.manual_trigger = ManualTriggerHandler()

DistressEngine.__init__ = _distress_engine_init_with_sos
